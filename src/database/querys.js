...
export const queries = {
    ...
    'values (SUBSTRING((SELECT TipoSolicitud from TipoSolicitud where IDTipo=@IdTipoSolicitud),1,1) + (SELECT (CONVERT(VARCHAR,GETDATE(),112)) + (SELECT (CONVERT(VARCHAR,DATEPART(hh, getdate())))) + (SELECT(CONVERT(VARCHAR,DATEPART(n, getdate())))) + (SELECT(CONVERT(VARCHAR,DATEPART(s, getdate())))) ), '+
